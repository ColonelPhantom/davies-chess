...
use crate::search::{NodeCount, alphabeta};
...
pub fn bench() {
    for (fen, depth) in POSITIONS {
        ...
        let mut count = NodeCount { nodes: 0, leaves: 0, qnodes: 0 };
        let mut tt = Vec::new(); tt.resize_with(1 << 20, || AtomicU64::new(0));
        let (score, pv) = alphabeta(position, depth, -32000, 32000, &mut count, &mut tt);
        println!("FEN: {}", fen);
