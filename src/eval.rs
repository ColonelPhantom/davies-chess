...
pub fn eval_piece(sq: Square, color: Color, role: Role) -> i16 {
    ...
    // use middle-game for this
    let pst_value = PST[piece_idx - 1][0][sq_idx];
...
pub fn eval(position: &shakmaty::Chess) -> i16 {
    ...
    for (sq, piece) in position.board() {
        let sq_rel = if piece.color == Color::White {sq.flip_vertical()} else {sq};
        let base_mg_val = match piece.role {
            Role::Pawn => 82,
            Role::Knight => 337,
            Role::Bishop => 365,
            Role::Rook => 477,
        ...
        let base_eg_val = match piece.role {
            Role::Pawn => 94,
            Role::Knight => 281,
            Role::Bishop => 297,
            Role::Rook => 512,
        ...
        mg[piece.color as usize] += base_mg_val + PST[piece.role as usize - 1][0][sq_rel as usize];
        eg[piece.color as usize] += base_eg_val + PST[piece.role as usize - 1][1][sq_rel as usize];
    ...
    let mg = mg[position.turn() as usize] - mg[position.turn().other() as usize];
    let eg = eg[position.turn() as usize] - eg[position.turn().other() as usize];

    let mg_weight = phase.clamp(0, 24);
    let eg_weight = 24 - mg_weight;

    ((mg * mg_weight) + (eg * eg_weight)) / 24
